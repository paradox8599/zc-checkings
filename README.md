...
5. 「加班统计」tab 可把当前选中月份导出成一份「加班申请单」xlsx，详见下方「加班申请单导出」。
...
- 加班：由插件实时算出 `{日期, 加班分钟}`，叠加每日修正后使用，不落库
- 每日修正：用来对齐「插件算出来的数」和「已提交的加班表」。在「加班统计」tab 的每日明细里，每行右侧的「修正」框可对**当天加班**加减一个分钟数（可为负，留空或填 `0` 即清除）；存 localStorage（key `zc-day-adjust`），形如 `{"2026-08-15": 32}`。修正直接并入当天加班，月合计与抵扣池随之变化
- 已上报月份：只有上报给公司的加班才能用来抵扣请假。在「加班统计」tab 的表头勾选「已上报」，该月修正后的加班才计入可抵扣时间；存 localStorage（key `zc-month-reported`），形如 `["2026-08"]`。没勾的月份不计入抵扣池，只在台账里以「未上报」单列
...
7. 某天加班被修正为负数时不做抵扣，而是从该月起冲减最早的可抵扣加班，声明里单独注明
...
「加班统计」tab 里的「导出申请表」按钮，按**当前选中的月份**生成一份 xlsx。表格以一张加班申请单为模板（保留原表的合并单元格、边框与样式）。

- 「加班时间」一栏按模板格式逐日列出：`自 yyyy 年 MM 月 DD 日 HH:MM 时至 … 时，共计 X h YY m`，末尾 `总计 …`；只列有加班的日期，数值由插件算法算出并叠加每日修正
- 申请人、所属部门、所属项目、加班原因、文件名在「设置」区维护，改动即时保存；存 localStorage（key `zc-form-config`）
- 文件名默认按月份生成（`yyyy年m月加班申请单`，填了申请人则追加 `-申请人`）；切换月份或改申请人时重新生成，也可手填覆盖
- 模板在 `src/assets/overtime-form.xlsx`，只保留表单结构（示例里的个人信息已清空成占位符），构建时内联进脚本
...
- `pnpm run release`：发布版，输出 `dist/attendance.release.user.js`（约 100KB 自包含，core 内联、无本地服务依赖），**可直接拖入 Tampermonkey 安装**
...
src/form.ts      加班申请单：按模板生成 xlsx（fflate 解压/打包）
src/panel.ts     浮动面板 UI
src/assets/      加班申请单模板（overtime-form.xlsx）
...
- 运行期：`fflate`（打包进脚本，用于解压/打包加班申请单 xlsx 模板）
