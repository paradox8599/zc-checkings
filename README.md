...
| 上班时间 | `08:30` | 正常上班时间 |
| 下班时间 | `17:30` | 正常下班时间 |
| 午休(分钟) | `60` | 午休时长（从工时中扣除） |
| 下班(分钟)内不算加班 | `30` | 下班宽限，超出该分钟数才计加班（仅 `threshold` 模式用） |
| 少于(分钟)不算加班 | `60` | 加班不足该分钟数时整天不计 |
| 加班计算 | 8hours | `threshold`=下班超过 标准下班+宽限 才计；`standard`=下班超过 标准下班 就计；`8hours`=工作日工时超过 8 小时的部分 |
| 周末扣午休 | 否 | 周末是否扣午休（默认不扣） |

周末加班按 8 小时封顶（不超过一个标准工作日）。
